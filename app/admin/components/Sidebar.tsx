...
export default function Sidebar({
  ...
  const NavItem = ({ item, isSub = false }: { item: any; isSub?: boolean }) => {
    ...
    return (
      <Link
        ...
        className={`${baseStyles} ${activeStyles} ${isSub ? "text-sm" : ""}`}
      >
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white flex flex-col transition-transform duration-300 lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
                  ...
                  {openSubmenus[item.title] && (
                    <div className="ml-4 mt-1 border-l border-slate-800 pl-2 space-y-1">
                      {item.submenu.map((sub) => (
                        <NavItem key={sub.href} item={sub} isSub />
                      ))}
                    </div>
                  )}
                </>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          <div className="flex items-center space-x-3 mb-4 px-2">
            ...
            <div className="truncate font-medium text-sm text-slate-300">
              {user.name}
            </div>
          </div>
          <button
            onClick={logout}
            className="flex items-center space-x-3 px-4 py-2.5 text-red-400 hover:text-red-300 w-full hover:bg-red-400/10 rounded-lg transition-colors"
          >
